...
class RobertaNLIHier(nn.Module):
    ...
    def __init__(
        ...
        stage1_class_weights: Optional[torch.Tensor] = None,  # shape [2]
        stage2_class_weights: Optional[torch.Tensor] = None,  # shape [2]
        lambda2: float = 0.7,  # downweight stage2 loss to protect Ambivalent
    ):
        super().__init__()
    ...
    def forward(
        ...
        stage2_labels=None,  # [B] in {0,1} but only valid where stage1_labels==0
        stage2_mask=None,    # [B] bool, True where we compute stage2 loss
    ):
...
def load_qevasion_with_nli(data_dir: str = "data/qevasion_with_nli"):
    path = Path(data_dir)
    if not path.exists():
        raise RuntimeError(f"Cannot find '{data_dir}'. Run precompute_nli_features.py first.")
    return load_from_disk(str(path))


def load_external_with_nli(external_dir: str):
    """
    Load an external dataset that you saved_to_disk and which ALREADY contains:
      - interview_question, interview_answer
      - p_contra, p_neutral, p_entail
    Optionally it may include clarity_label (if you want evaluation), otherwise predictions-only.
    """
    path = Path(external_dir)
    if not path.exists():
        raise RuntimeError(f"Cannot find external dataset at '{external_dir}'.")
    return load_from_disk(str(path))
...
def tokenize_dataset(dataset, tokenizer, label2id, max_length: int = 256):
    """
    Works for datasets WITH labels.
    Requires columns: interview_question, interview_answer, clarity_label, p_contra, p_neutral, p_entail
    """
...
def tokenize_dataset_no_labels(dataset, tokenizer, max_length: int = 256):
    """
    For prediction-only datasets WITHOUT clarity_label.
    Requires columns: interview_question, interview_answer, p_contra, p_neutral, p_entail
    """
    def encode_batch(batch):
        enc = tokenizer(
            batch["interview_question"],
    ...
    encoded = dataset.map(encode_batch, batched=True)
    cols = ["input_ids", "attention_mask", "p_contra", "p_neutral", "p_entail"]
    encoded.set_format(type="torch", columns=cols)
    return encoded
...
def make_stage_targets(labels_3way: torch.Tensor, id2label: Dict[int, str]):
    """
    stage1: reply=1 iff Clear Reply else 0
    stage2: within non-reply-ish (stage1==0): CNR=1 iff Clear Non-Reply else 0 (Ambivalent=0)
    """
...
def compute_stage_class_weights(train_split, label2id, id2label, device, cap: float = 3.0, use_sqrt: bool = True):
    """
    Inverse-frequency weights (optionally sqrt + cap) for:
      stage1: NonReplyish=0, Reply=1
      stage2: Ambivalent=0, ClearNonReply=1  (only for stage1==0 examples)
    """
    from collections import Counter

    y = [label2id[lbl] for lbl in train_split["clarity_label"]]
    y = torch.tensor(y)

    stage1_cpu = torch.tensor([1 if id2label[i.item()] == "Clear Reply" else 0 for i in y])
    stage2_cpu = torch.tensor([1 if id2label[i.item()] == "Clear Non-Reply" else 0 for i in y])
    ...
    print("Stage2 weights (Ambivalent=0, ClearNonReply=1):", w2.tolist(), "counts:", dict(c2))
    return w1, w2
...
def predict_thresholded(
    ...
    If P(reply) > t_reply -> Clear Reply
    else:
      If P(cnr) > t_cnr -> Clear Non-Reply
      else -> Ambivalent

    Returns predicted LABEL IDS in the same label2id mapping used in training.
    """
    model.eval()
    ...
    for batch in dataloader:
        ...
        p1 = F.softmax(out["logits1"], dim=-1)  # [B,2] => p(nonreplyish), p(reply)
        p2 = F.softmax(out["logits2"], dim=-1)  # [B,2] => p(amb), p(cnr)
        ...
        for i in range(p_reply.size(0)):
            if float(p_reply[i].item()) > t_reply:
                ...
                if float(p_cnr[i].item()) > t_cnr:
                    preds.append(inv["Clear Non-Reply"])
                else:
                    preds.append(inv["Ambivalent"])
...
def evaluate_thresholded(model, dataloader, device, id2label, t_reply, t_cnr, title="Eval") -> Dict[str, float]:
    ...
    report_str = classification_report(labels, preds, target_names=target_names, digits=3)
...
def tune_thresholds_on_validation(
    ...
    for t_reply in reply_grid:
        for t_cnr in cnr_grid:
            ...
            if f1 > best_f1:
                best_f1 = f1
                best_t_reply = t_reply
                best_t_cnr = t_cnr

    print("\nBEST thresholds on validation:")
    print(f"  macro_f1={best_f1:.4f}, t_reply={best_t_reply:.2f}, t_cnr={best_t_cnr:.2f}")
...
def train_clarity_with_nli_hier(
    ...
    lambda2: float = 0.7,
    model_nickname: str = "roberta_hier_nli",
):
    ...
    print("Splitting train/validation (90/10) from QEvasion train...")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # Quick evaluation using a default thresholds (not tuned yet)
        metrics = evaluate_thresholded(model, val_loader, device, id2label, t_reply=0.70, t_cnr=0.70, title="Val (default thr)")
        val_f1 = metrics["macro_f1"]

        history.append({"epoch": epoch, "train_loss": tr_loss, "val_macro_f1_default_thr": val_f1})
    ...
    if best_state is not None:
        ...
        print(f"\nSaved best checkpoint: {ckpt}")

    # Tune thresholds on validation (THIS is the big win)
    best_t_reply, best_t_cnr, best_thr_f1 = tune_thresholds_on_validation(model, val_loader, device, id2label)

    # Final reports with tuned thresholds
    val_metrics = evaluate_thresholded(model, val_loader, device, id2label, best_t_reply, best_t_cnr, title="Val (tuned thr)")
    test_metrics = evaluate_thresholded(model, test_loader, device, id2label, best_t_reply, best_t_cnr, title="QEvasion Test (tuned thr)")
    ...
    summary = {
        ...
        "lambda2": lambda2,
        "use_stage_weights": use_stage_weights,
        "best_val_macro_f1_default_thr": best_val_f1,
        "best_thresholds": {"t_reply": best_t_reply, "t_cnr": best_t_cnr, "val_macro_f1": best_thr_f1},
    ...
    return model, tokenizer, label2id, id2label, test_loader, (best_t_reply, best_t_cnr)
...
if __name__ == "__main__":
    # Train + tune thresholds on QEvasion
    model, tokenizer, label2id, id2label, test_loader, (t_reply, t_cnr) = train_clarity_with_nli_hier(
        ...
        lambda2=0.7,
        model_nickname="roberta_hier_nli_thr",
    )
