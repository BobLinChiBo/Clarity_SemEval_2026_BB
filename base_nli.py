...
class RobertaWithNLI(nn.Module):
    def __init__(self, model_name: str, num_labels: int, nli_feat_dim: int = 3, dropout_prob: float = 0.1):
        super().__init__()
        self.encoder = AutoModel.from_pretrained(model_name)
        hidden_size = self.encoder.config.hidden_size
        self.dropout = nn.Dropout(dropout_prob)
        self.classifier = nn.Linear(hidden_size + nli_feat_dim, num_labels)
    ...
    def forward(
        ...
        if nli_feats is not None:
            x = torch.cat([pooled, nli_feats], dim=-1)
        else:
            x = pooled
...
def train_clarity_with_nli(
    ...
    model = RobertaWithNLI(model_name=model_name, num_labels=num_labels, nli_feat_dim=3)
    model.to(device)
